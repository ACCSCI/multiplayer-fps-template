...
export class BuyMenu {
    ...
    refresh(playerData, teamName) {
        const data = JSON.stringify([
            playerData.money,
            playerData.slots,
            playerData.isAttacker,
            playerData.armor,
            playerData.armorType,
        ]);
        ...
        const grenadeCount = Enum.GrenadeSlots.reduce(
            (sum, slot) => (sum += playerData.slots[slot] ? playerData.slots[slot].pcs || 1 : 0),
            0,
        );
        ...
        ${
            playerData.slots[Enum.InventorySlot.SLOT_GRENADE_FLASH] === undefined ||
            playerData.slots[Enum.InventorySlot.SLOT_GRENADE_FLASH].pcs < 2
