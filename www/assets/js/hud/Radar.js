...
export class Radar {
    ...
    setZoom(zoom, keepCentered) {
        ...
        this.#borderPadding =
            keepCentered || zoom > 3 ? 0 : Math.ceil((4 - Math.min(3, zoom)) * (this.#mapSize.radarImgBoundary / 11));
    }

    update(myTeamPlayers, idSpectator, spectatorRotationHorizontal, bombPosition) {
        let spectator;
        let _bombHasPlayer = false;
        const ctx = this.#ctx;
        ...
        const meX = Math.min(
            Math.max(this.#mapSize.boundary.min[0] + this.#borderPadding, spectator.data.position.x),
            this.#mapSize.boundary.max[0] - this.#borderPadding,
        );
        const meY = Math.min(
            Math.max(this.#mapSize.boundary.min[1] + this.#borderPadding, spectator.data.position.z),
            this.#mapSize.boundary.max[1] - this.#borderPadding,
        );
    ...
    #checkEnemySpotted(myTeamPlayer, ctx) {
        ...
        this.#camera.rotation.set(
            Utils.serverVerticalRotationToThreeRadian(myTeamPlayer.data.look.vertical),
            Utils.serverHorizontalRotationToThreeRadian(myTeamPlayer.data.look.horizontal),
            0,
        );
        ...
        this.#game.players.forEach((player) => {
            ...
            if (
                intersects.length === 0 ||
                intersects[0].distance + 50 < matePosition.distanceTo(player.getSightPositionThreeVector())
            ) {
                return;
            }
