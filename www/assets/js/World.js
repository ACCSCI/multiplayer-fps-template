...
export class World {
    ...
    init(mapName, setting) {
        ...
        if (!setting.shouldPreferPerformance()) {
            new RGBELoader().load("./resources/img/kloofendal_48d_partly_cloudy_puresky_1k.hdr", (texture) => {
    ...
    itemAttack(player, item, isSpectator) {
        ...
        if (sparkFeedbackSlots.includes(item.slot)) {
            ...
            if (isSpectator) {
                spark = player
                    .get3DObject()
                    .getObjectByName("pov-item")
                    ?.getObjectByName(`item-${item.id}`)
                    ?.getObjectByName("pov-spark");
            } else {
            ...
            if (spark) {
                ...
                mesh.scale.setScalar(
                    (item.slot === Enum.InventorySlot.SLOT_PRIMARY ? 12 : 8) * (isSpectator ? 0.85 : 1),
                );
                spark.add(mesh);
    ...
    spawnFlame(position, size, height) {
        ...
        const material = this.loadCache(
            `flame-mat-${lightnessValue}`,
            () =>
    ...
    spawnSmoke(position, height, smokeId) {
        ...
        const geo = new THREE.CylinderGeometry(
            size - Utils.randomInt(0, offset),
            size,
            height - Utils.randomInt(0, offset),
            Utils.randomInt(5, 22),
        );
        geo.translate(position.x, position.y + geo.parameters.height / 2, -position.z);
        this.#smokes[smokeId].geometries.push(geo);

        const len = this.#smokes[smokeId].geometries.length;
        if (len % 10 !== 0 && len < this.#smokes[smokeId].count) {
            return;
        }

        const geometry = BufferGeometryUtils.mergeGeometries(this.#smokes[smokeId].geometries);
    ...
    bulletWallHit(origin, hitPosition, item) {
        ...
        const radius =
            item.slot === Enum.InventorySlot.SLOT_PRIMARY
                ? 1.8
                : item.slot === Enum.InventorySlot.SLOT_SECONDARY
                  ? 1.5
                  : 1.1;
    ...
    playSound(soundName, position, inPlayerHead, refDistance = 1) {
        ...
        this.#audioLoader.load(`./resources/sound/${soundName}`, (buffer) => {
