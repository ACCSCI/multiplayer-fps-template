import { GrenadeSlots, InventorySlot, ItemId, SoundType } from "./Enums.js";
import { Utils } from "./Utils.js";

export class SoundRepository {
    #alwaysInHeadTypes = [SoundType.BOMB_PLANTED, SoundType.BOMB_DEFUSED];
    #myPlayerTypes = [
        SoundType.ITEM_RELOAD,
        SoundType.ITEM_PICKUP,
        SoundType.ITEM_ATTACK,
        SoundType.ITEM_ATTACK2,
        SoundType.ITEM_BUY,
        SoundType.PLAYER_STEP,
        SoundType.ATTACK_NO_AMMO,
        SoundType.BOMB_PLANTING,
        SoundType.BOMB_DEFUSING,
    ];
    #soundPlayer;
    ...
    play(data, spectatorId, tickId) {
        const soundName = this.#getSoundName(data.type, data.item, data.player, spectatorId, tickId);
        if (!soundName) {
            return;
        }

        const inPlayerSpectateHead =
            this.#alwaysInHeadTypes.includes(data.type) ||
            (data.player && data.player === spectatorId && this.#myPlayerTypes.includes(data.type));
    ...
    #getSoundName(type, item, playerId, playerSpectateId, tickId) {
        ...
        if (type === SoundType.BULLET_HIT_HEADSHOT) {
            return playerId === playerSpectateId
                ? "249821__spookymodem__weapon-blow.wav"
                : "632704__adh-dreaming__fly-on-the-wall-snare.wav";
        }
        ...
        if (
            type === SoundType.FLAME_SPAWN ||
            type === SoundType.FLAME_EXTINGUISH ||
            type === SoundType.SMOKE_SPAWN ||
            type === SoundType.SMOKE_FADE
        ) {
            // fixme make some noise
            return null;
        }
        ...
        console.warn("No song defined for", type, item, playerId, playerSpectateId, tickId);
        return null;
    }
