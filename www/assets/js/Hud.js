...
export class HUD {
    ...
    #grenadeSlots = [
        Enum.InventorySlot.SLOT_GRENADE_DECOY,
        Enum.InventorySlot.SLOT_GRENADE_HE,
        Enum.InventorySlot.SLOT_GRENADE_MOLOTOV,
        Enum.InventorySlot.SLOT_GRENADE_SMOKE,
        Enum.InventorySlot.SLOT_GRENADE_FLASH,
    ];
    ...
    bombPlanted(detonationTimeSec) {
        ...
        this.displayBottomMessage(
            `<span class="text-danger">⚠️ Alert</span><br>The bomb has been planted.<br>${detonationTimeSec} seconds till detonation.`,
        );
    ...
    #startCountDown(timeMs) {
        ...
        const roundTimeInterval = setInterval(() => {
            roundTimeSec--;

            const roundTimeMinute = Math.floor(roundTimeSec / 60);
    ...
    equip(slotId, _itemId, availableSlots) {
        let html = "";
        ...
        this.#grenadeSlots.forEach((slot) => {
            ...
            const grenadeCount = item.pcs > 1 ? `<span class="count">x${item.pcs}</span>` : "";
    ...
    updateHud(player) {
        ...
        this.#elements.haveDefuseKit.classList.toggle(
            "hidden",
            player.slots[Enum.InventorySlot.SLOT_KIT] === undefined,
        );
        this.#elements.haveBomb.classList.toggle("hidden", player.slots[Enum.InventorySlot.SLOT_BOMB] === undefined);
        this.#elements.spectateUi.classList.toggle(
            "hidden",
            this.#game.playerMe.getId() === this.#game.playerSpectate.getId(),
        );
        ...
        const myTeamIndex = this.#game.playerMe.getTeamIndex();
        const otherTeamIndex = this.#game.playerMe.getOtherTeamIndex();
    ...
    `;
        ...
        setting.addUpdateCallback("hudColorShadow", (newValue) =>
            elementHud.style.setProperty("--hud-color-shadow", newValue),
        );
        ...
        radarImage.onload = function () {
            ...
            setting.addUpdateCallback("radarZoom", (newValue) =>
                self.#radar.setZoom(parseFloat(newValue), setting.getRadarAlwaysCentered()),
            );
            setting.update("radarZoom", setting.getRadarZoom());
        };
        radarImage.src = `./resources/map/${map}.png`;

        this.#elements.buyMenu.addEventListener(
            "click",
            (e) => {
