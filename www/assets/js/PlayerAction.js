...
export class PlayerAction {
        ...
        if (!isKeyDown && actionIndex.startsWith("buy ")) {
            ...
            if (match[2]) {
                match[2].split(",").forEach((value) => {
                    const buyItemId = parseInt(value, 10);
                    if (buyItemId) {
    ...
    #loadCallbacks(game, hud) {
        ...
        this.#actionCallback[Action.JUMP] = (enabled) => {
            if (enabled) {
        ...
        this.#actionCallback[Action.CROUCH] = (enabled) => {
            if (enabled) {
        ...
        this.#actionCallback[Action.WALK] = (enabled) => {
            if (enabled) {
        ...
        this.#actionCallback[Action.DROP] = (enabled) => {
            if (enabled) {
        ...
        this.#actionCallback[Action.EQUIP_MOLOTOV] = (enabled) =>
            enabled && this.equip(InventorySlot.SLOT_GRENADE_MOLOTOV);
    ...
    getPlayerAction(game, userSetting) {
        ...
        if (game.buyList.length) {
            game.buyList.forEach((buyMenuItemId) => {
        ...
        if (this.#states.attack) {
            ...
            this.#states.spraying &&
            this.#states.sprayTriggerStartMs &&
            this.#states.sprayTriggerStartMs + userSetting.getSprayTriggerDeltaMs() < Date.now()
        ) {
            action.push(this.#rotationToServerLook(game.getPlayerMeRotation()));
            action.push("attack");
        } else {
            const look = this.#rotationToServerLook(game.getPlayerMeRotation());
